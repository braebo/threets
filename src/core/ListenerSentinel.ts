...
export class LinkedListener<T extends any = any> {
	...
	constructor(
		...
		{ ctx = this, once = false }: LinkedListenerOptions = {},
	) {
		this.ctx = ctx
		this._once = once
	}
	...
	static create<T extends any>(): LinkedListener<T> {
		const sentinel = new LinkedListener<T>(null, {
			once: false,
		})
	...
	add(cb: EventCallback<T>): void {
		this.connect(
			new LinkedListener(cb, {
				ctx: this,
			}),
		)
	}
