...
class Vector2 {
	...
	addScaledVector(v: Vec2, s: number) {
		this.x += v.x * s
		this.y += v.y * s
	...
	fromArray(array: [number, number], offset = 0) {
		this.x = array[offset]
	...
	toArray(array = [] as any as [number, number], offset = 0): [number, number] {
		array[offset] = this.x
	...
	fromBufferAttribute(
		attribute: { getX: (arg0: number) => number; getY: (arg0: number) => number },
		index: number,
	) {
