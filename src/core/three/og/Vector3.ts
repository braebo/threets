...
export class Vector3 {
	...
	lerpVectors(v1: Vec3, v2: Vec3, alpha: number) {
	...
	crossVectors(a: this, b: { x: any; y: any; z: any }) {
		const ax = a.x,
			ay = a.y,
			az = a.z
		const bx = b.x,
	...
	setFromCylindrical(c: { radius: number; theta: number; y: number }) {
	...
	setFromMatrixColumn(m: { elements: any }, index: number): this {
		return this.fromArray(m.elements, index * 4)
	}

	setFromMatrix3Column(m: { elements: any }, index: number): this {
	...
	toArray(array = [] as unknown as [x, y, z], offset = 0): [x, y, z] {
		array[offset] = this.x
