...
export class Box3 {
	...
	containsBox(box: { min: Vec3; max: Vec3 }) {
		return (
	...
	getParameter(point: Vec3, target: Vector3) {
	...
	intersectsBox(box: { max: Vec3; min: Vec3 }) {
	...
	intersectsTriangle(triangle: { a: Vec3; b: Vec3; c: Vec3 }) {
		if (this.isEmpty()) {
			return false
		}
	...
	clampPoint(point: any, target: Vector3) {
	...
	getBoundingSphere(target: { makeEmpty: () => void; center: any; radius: number }) {
		if (this.isEmpty()) {
			target.makeEmpty()
		} else {
	...
	intersect(box: { min: Vec3; max: Vec3 }) {
	...
	applyMatrix4(matrix: { elements: any }) {
	...
	equals(box: {
		min: { equals: (arg0: Vector3) => any }
		max: { equals: (arg0: Vector3) => any }
	}) {
